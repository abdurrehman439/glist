...
function simpleMap(lat_element,_latitude,long_element,_longitude, element, markerDrag, place){
    ...
    if( place ){
        ...
        drawMap(mapCenter, lat_element, long_element);
    }

    function drawMap(mapCenter, lat_element, long_element){
        var mapOptions = {
            zoom: 14,
            center: mapCenter,
        ...
        var marker = new RichMarker({
            ...
            content: "<img src='" + base_url + "assets/img/marker.png'>",
            flat: true
        });
        google.maps.event.addListener(marker, "dragend", function () {
            var latitude = this.position.lat();
            var longitude = this.position.lng();
            $('#'.lat_element).val( this.position.lat() );
            $('#'.long_element).val( this.position.lng() );
        });

        autoComplete(map, marker, lat_element, long_element);
    }
...
function autoComplete(map, marker, lat_element, long_element){
    if( $("#zipcode").length ){
        if( !map ){
            map = new google.maps.Map(document.getElementById("zipcode"));
        }
        ...
        google.maps.event.addListener(autocomplete, 'place_changed', function() {
            ...
            if( marker ){
                ...
                $('#'+lat_element).val( marker.getPosition().lat() );
                $('#'+long_element).val( marker.getPosition().lng() );
            }
            var address = '';
        ...
        function success(position) {
            ...
            $('#'+lat_element).val( position.coords.latitude );
            $('#'+long_element).val( position.coords.longitude );
        }
