...
class Rent extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/rent-page',$user);
    ...
    public function insert(){
        ...
        $this->form_validation->set_rules('price-duration', 'Price Type', 'required');
        ...
        $this->form_validation->set_rules('rent-category', 'Category', 'required|min_length[3]|max_length[255]');
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_rent.jpg"
            );
            ...
            $this->RentModel->insert_rent($path);

            echo json_encode( array('success' => "Rent Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
            ...
            $this->RentModel->edit_rent($path);

            echo json_encode( array('success' => "Rent Listing has been Edited.....!") );
        }
    }
    ...
    public function filters(){
        if( $this->input->post('index') == 0 ){
            ...
            {
                $query = $this->RentModel->category_filter($this->input->post('category'));
            }
            else
            {
            ...
            if( sizeof($this->input->post('category')) > 0 )
            {
                $query = $this->RentModel->loadmore_category_filter($this->input->post('category'),$this->input->post('index'));
            }
            else
            {
                return print_r(json_encode(array('result'=>"No results found")));
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->RentModel->no_filter();
        }
        else{
            $query = $this->RentModel->loadmore_no_filter($this->input->post('index'));
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->RentModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->RentModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->RentModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/rent-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->RentModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->RentModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested Rent post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
