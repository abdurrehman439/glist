...
class Restaurants extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/restaurants-page',$user);
    ...
    public function insert(){
        ...
        $this->form_validation->set_rules('restaurants-category', 'Category', 'required');
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_restaurants.jpg"
            );
            ...
            $this->RestaurantsModel->insert_restaurants($path);

            echo json_encode( array('success' => "Restaurants Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
            ...
            $this->RestaurantsModel->edit_restaurants($path);

            echo json_encode( array('success' => "Restaurants Listing has been Edited.....!") );
        }
    }
    ...
    public function filters(){
        if( $this->input->post('index') == 0 ){
            ...
            {
                $query = $this->RestaurantsModel->category_filter($this->input->post('category'));
            }
            else
            {
            ...
            if( sizeof($this->input->post('category')) > 0 )
            {
                $query = $this->RestaurantsModel->loadmore_category_filter($this->input->post('category'),$this->input->post('index'));
            }
            else
            {
                return print_r(json_encode(array('result'=>"No results found")));
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->RestaurantsModel->no_filter();
        }
        else{
            $query = $this->RestaurantsModel->loadmore_no_filter($this->input->post('index'));
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->RestaurantsModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->RestaurantsModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->RestaurantsModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/restaurant-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->RestaurantsModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->RestaurantsModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested Restaurants post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
