...
class Funding extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/funding-page',$user);
    ...
    public function insert(){
		...
		$this->form_validation->set_rules('amount', 'Amount', 'required|greater_than[0]|min_length[1]');
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_funding.jpg"
            );
            ...
            $this->FundingModel->insert_funding($path);

            echo json_encode( array('success' => "Funding Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
            ...
            $this->FundingModel->edit_funding($path);

            echo json_encode( array('success' => "Funding Listing has been Edited.....!") );
        }
    }
    ...
    public function filters(){
        if( $this->input->post('index') == 0 ){
            ...
            {
                $query = $this->FundingModel->category_filter($this->input->post('category'));
            }
            else
            {
            ...
            if( sizeof($this->input->post('category')) > 0 )
            {
                $query = $this->FundingModel->loadmore_category_filter($this->input->post('category'),$this->input->post('index'));
            }
            else
            {
                return print_r(json_encode(array('result'=>"No results found")));
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->FundingModel->no_filter();
        }
        else{
            $query = $this->FundingModel->loadmore_no_filter($this->input->post('index'));
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->FundingModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->FundingModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->FundingModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/funding-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->FundingModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->FundingModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested Funding post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
