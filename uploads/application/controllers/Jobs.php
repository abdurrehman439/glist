...
class Jobs extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/jobs-page',$user);
    ...
    public function insert(){
        ...
        $this->form_validation->set_rules('salary', 'Salary', 'required|numeric|greater_than[0]|max_length[10]');
        ...
        $this->form_validation->set_rules('jobs-type', 'Type', 'required|max_length[255]');
        $this->form_validation->set_rules('jobs-status', 'Status', 'required|max_length[255]');
        $this->form_validation->set_rules('jobs-industry', 'Industry', 'required|max_length[255]');
        $this->form_validation->set_rules('jobs-function', 'Function', 'required|max_length[255]');
        $this->form_validation->set_rules('jobs-experience', 'Experience', 'required|numeric|greater_than[0]|max_length[10]');
        $this->form_validation->set_rules('jobs-education', 'Education', 'required|min_length[3]|max_length[255]');
        $this->form_validation->set_rules('jobs-skill', 'Skill', 'required|min_length[3]|max_length[255]');
        $this->form_validation->set_rules('jobs-software', 'Software', 'required|min_length[3]|max_length[255]');
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_jobs.jpg"
            );
            ...
            $this->JobsModel->insert_jobs($path);

            echo json_encode( array('success' => "Jobs Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
            ...
            $this->JobsModel->edit_jobs($path);

            echo json_encode( array('success' => "Jobs Listing has been Edited.....!") );
        }
    }

    public function filters(){
        if( $this->input->post('index') == 0 ){
            $query = $this->JobsModel->filter();
        }
        else{
            $query = $this->JobsModel->loadmore_filter($this->input->post('index'));
        }
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->JobsModel->no_filter();
        }
        else{
            $query = $this->JobsModel->loadmore_no_filter($this->input->post('index'));
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->JobsModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->JobsModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->JobsModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/job-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->JobsModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->JobsModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested Jobs post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
