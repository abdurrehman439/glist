...
class Sales extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/sales-page',$user);
    ...
    public function insert(){
        ...
        $this->form_validation->set_rules('sales-category', 'Category', 'required');
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_sales.jpg"
            );
            ...
            $this->SalesModel->insert_sales($path);

            echo json_encode( array('success' => "Sales Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
            ...
            $this->SalesModel->edit_sales($path);

            echo json_encode( array('success' => "Sales Listing has been Edited.....!") );
        }
    }

    public function filters(){
        if( $this->input->post('index') == 0 ){
            $query = $this->SalesModel->filter();
        }
        else{
            $query = $this->SalesModel->loadmore_filter($this->input->post('index'));
        }
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->SalesModel->no_filter();
        }
        else{
            $query = $this->SalesModel->loadmore_no_filter($this->input->post('index'));
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->SalesModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->SalesModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->SalesModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/sale-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->SalesModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->SalesModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested Sales post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
