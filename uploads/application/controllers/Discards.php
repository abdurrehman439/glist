...
class Discards extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/discards-page',$user);
    ...
    public function insert(){
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_discards.jpg"
            );
            ...
            $this->DiscardsModel->insert_discards($path);

            echo json_encode( array('success' => "Discards & Scraps Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
            ...
            $this->DiscardsModel->edit_discards($path);

            echo json_encode( array('success' => "Discards & Scraps Listing has been Edited.....!") );
        }
    }
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->DiscardsModel->no_filter();
        }
        else{
            $query = $this->DiscardsModel->loadmore_no_filter($this->input->post('index'));
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->DiscardsModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->DiscardsModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->DiscardsModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/discard-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->DiscardsModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->DiscardsModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested Discards post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
