...
class Coupons extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/coupons-page',$user);
    ...
    public function insert(){
        ...
        $this->form_validation->set_rules('finish-date', 'Finish Date', 'required');
        $this->form_validation->set_rules('coupons-category', 'Category', 'required');
        $this->form_validation->set_rules('original-price', 'Original Price', 'required|numeric|greater_than[0]|max_length[10]');
        $this->form_validation->set_rules('discounted-price', 'Discounted Price', 'required|numeric|greater_than[0]|max_length[10]');
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_coupons.jpg"
            );
            ...
            $this->CouponsModel->insert_coupons($path);

            echo json_encode( array('success' => "Coupons Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
            ...
            $this->CouponsModel->edit_coupons($path);

            echo json_encode( array('success' => "Coupons Listing has been Edited.....!") );
        }
    }

    public function filters(){
        if( $this->input->post('index') == 0 ){
            $query = $this->CouponsModel->filter();
        }
        else{
            $query = $this->CouponsModel->loadmore_filter($this->input->post('index'));
        }
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->CouponsModel->no_filter();
        }
        else{
            $query = $this->CouponsModel->loadmore_no_filter($this->input->post('index'));
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->CouponsModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->CouponsModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->CouponsModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/coupon-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->CouponsModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->CouponsModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested Coupons post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
