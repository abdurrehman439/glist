...
class HireAResource extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/hirearesource-page',$user);
    ...
    public function insert(){
        ...
        $this->form_validation->set_rules('hire-a-resource-category', 'Category', 'required');
        $this->form_validation->set_rules('hourly-rate', 'Hourly Rate', 'required|numeric|greater_than[0]|max_length[10]');
        $this->form_validation->set_rules('details', 'Details', 'required|min_length[3]|max_length[255]');
        $this->form_validation->set_rules('hire-a-resource-country', 'Country', 'required');
        ...
        $this->form_validation->set_rules('hire-a-resource-second-language', 'Second Language', 'required');
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_hirearesource.jpg"
            );
            ...
            $this->HireAResourceModel->insert_hire_a_resource($path);

            echo json_encode( array('success' => "Gigs & Freelancers Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
            ...
            $this->HireAResourceModel->edit_hire_a_resource($path);

            echo json_encode( array('success' => "Gigs & Freelancers Listing has been Edited.....!") );
        }
    }
    
    public function filters(){
        if( $this->input->post('index') == 0 ){
            $query = $this->HireAResourceModel->filter();
        }
        else{
            $query = $this->HireAResourceModel->loadmore_filter($this->input->post('index'));
        }
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->HireAResourceModel->no_filter();
        }
        else{
            $query = $this->HireAResourceModel->loadmore_no_filter($this->input->post('index'));
        }
	...
	public function zipcode_filter()
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->HireAResourceModel->no_filter();
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->HireAResourceModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->HireAResourceModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->HireAResourceModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/hirearesource-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->HireAResourceModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->HireAResourceModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested Gigs & Freelancers post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
