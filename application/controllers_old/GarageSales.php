...
class GarageSales extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/garagesales-page',$user);
    ...
    public function insert(){
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_garagesales.jpg"
            );
            ...
            $this->GarageSalesModel->insert_garage_sales($path);

            echo json_encode( array('success' => "Garage Sales Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
           ...
           if($this->input->post('previous-images') == null && $number_of_files ==1){
                 echo json_encode( array('validation_errors' => "<p>Please upload minimum single file!</p>") );
            exit();
           }
            ...
            $this->GarageSalesModel->edit_garage_sales($path);

            echo json_encode( array('success' => "Garage Sales Listing has been Edited.....!") );
        }
    }
    ...
    public function filters(){
        ...
        if( $this->input->post('index') == 0 ){
            $query = $this->GarageSalesModel->filter($date_array);
        }
        else{
            $query = $this->GarageSalesModel->loadmore_filter($date_array,$this->input->post('index'));
        }
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->GarageSalesModel->no_filter();
        }
        else{
            $query = $this->GarageSalesModel->loadmore_no_filter($this->input->post('index'));
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->GarageSalesModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->GarageSalesModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->GarageSalesModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/garagesale-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->GarageSalesModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->GarageSalesModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested Garage Sales post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
