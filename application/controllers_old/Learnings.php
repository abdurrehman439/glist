...
class Learnings extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/learnings-page',$user);
    ...
    public function insert(){
        ...
        $this->form_validation->set_rules('learnings-category', 'Category', 'required');
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_learnings.jpg"
            );
            ...
            $this->LearningsModel->insert_learnings($path);

            echo json_encode( array('success' => "Learning & Classes Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
            ...
            $this->LearningsModel->edit_learnings($path);

            echo json_encode( array('success' => "Learning & Classes Listing has been Edited.....!") );
        }
    }
    
    public function filters(){
        if( $this->input->post('index') == 0 ){
            $query = $this->LearningsModel->filter();
        }
        else{
            $query = $this->LearningsModel->loadmore_filter($this->input->post('index'));
        }
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->LearningsModel->no_filter();
        }
        else{
            $query = $this->LearningsModel->loadmore_no_filter($this->input->post('index'));
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->LearningsModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->LearningsModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->LearningsModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/learning-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->LearningsModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->LearningsModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested Learnings post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
