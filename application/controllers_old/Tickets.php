...
class Tickets extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/tickets-page',$user);
    ...
    public function insert(){
        ...
        $this->form_validation->set_rules('tickets-category', 'Category', 'required');
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_tickets.jpg"
            );
            ...
            $this->TicketsModel->insert_tickets($path);

            echo json_encode( array('success' => "Tickets Stores Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
            ...
            $this->TicketsModel->edit_tickets($path);

            echo json_encode( array('success' => "Tickets Stores Listing has been Edited.....!") );
        }
    }
    ...
    public function filters(){
        ...
        if( $this->input->post('index') == 0 ){
            $query = $this->TicketsModel->filter($date_array);
        }
        else{
            $query = $this->TicketsModel->loadmore_filter($date_array,$this->input->post('index'));
        }
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->TicketsModel->no_filter();
        }
        else{
            $query = $this->TicketsModel->loadmore_no_filter($this->input->post('index'));
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->TicketsModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->TicketsModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->TicketsModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/ticket-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->TicketsModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->TicketsModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested Tickets post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
