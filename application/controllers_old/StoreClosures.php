...
class StoreClosures extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/storeclosures-page',$user);
    ...
    public function insert(){
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_storeclosures.jpg"
            );
            ...
            $this->StoreClosuresModel->insert_store_closures($path);

            echo json_encode( array('success' => "Store Closures Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
            ...
            $this->StoreClosuresModel->edit_store_closures($path);

            echo json_encode( array('success' => "Store Closures Listing has been Edited.....!") );
        }
    }
    ...
    /*public function filters(){
        if( $this->input->post('index') == 0 ){
            ...
            {
                $query = $this->StoreClosuresModel->category_filter($this->input->post('category'));
            }
            else
            {
            ...
            if( sizeof($this->input->post('category')) > 0 )
            {
                $query = $this->StoreClosuresModel->loadmore_category_filter($this->input->post('category'),$this->input->post('index'));
            }
            else
            {
                return print_r(json_encode(array('result'=>"No results found")));
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->StoreClosuresModel->no_filter();
        }
        else{
            $query = $this->StoreClosuresModel->loadmore_no_filter($this->input->post('index'));
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->StoreClosuresModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->StoreClosuresModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->StoreClosuresModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/storeclosure-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->StoreClosuresModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->StoreClosuresModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested StoreClosures post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
                    $this->session->set_flashdata("error_message", "Requested StoreClosures post doesn't belongs to you!");
                    redirect("user/my_listing");
                }
            }
            else
            {
                $this->session->set_flashdata("error_message", "Requested StoreClosures post doesn't exists!");
                redirect("user/my_listing");
            }
        }
    }
