...
class Attractions extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/attractions-page',$user);
    ...
    public function insert(){
        ...
        $this->form_validation->set_rules('attractions-category', 'Category', 'required');
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_attractions.jpg"
            );
            ...
            $this->AttractionsModel->insert_attractions($path);

            echo json_encode( array('success' => "Attractions Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
            ...
            $this->AttractionsModel->edit_attractions($path);

            echo json_encode( array('success' => "Attractions Listing has been Edited.....!") );
        }
    }

    public function filters(){
        if( $this->input->post('index') == 0 ){
            $query = $this->AttractionsModel->filter();
        }
        else{
            $query = $this->AttractionsModel->loadmore_filter($this->input->post('index'));
        }
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->AttractionsModel->no_filter();
        }
        else{
            $query = $this->AttractionsModel->loadmore_no_filter($this->input->post('index'));
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->AttractionsModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->AttractionsModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->AttractionsModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/attraction-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->AttractionsModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->AttractionsModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested Attractions post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
