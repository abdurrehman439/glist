...
class BusinessSales extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/business-sales-page',$user);
    ...
    public function insert(){
		...
		$this->form_validation->set_rules('price', 'Price', 'required|greater_than[0]|min_length[1]');
        ...
        $this->form_validation->set_rules('business-sales-category', 'Category', 'required');
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_business-sales.jpg"
            );
            ...
            $this->BusinessSalesModel->insert_business_sales($path);

            echo json_encode( array('success' => "Business Sales Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
            ...
            $this->BusinessSalesModel->edit_business_sales($path);

            echo json_encode( array('success' => "Business Sales Listing has been Edited.....!") );
        }
    }
    ...
    public function filters(){
        if( $this->input->post('index') == 0 ){
            ...
            {
                $query = $this->BusinessSalesModel->category_filter($this->input->post('category'));
            }
            else
            {
            ...
            if( sizeof($this->input->post('category')) > 0 )
            {
                $query = $this->BusinessSalesModel->loadmore_category_filter($this->input->post('category'),$this->input->post('index'));
            }
            else
            {
                return print_r(json_encode(array('result'=>"No results found")));
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->BusinessSalesModel->no_filter();
        }
        else{
            $query = $this->BusinessSalesModel->loadmore_no_filter($this->input->post('index'));
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->BusinessSalesModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->BusinessSalesModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->BusinessSalesModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/business-sales-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->BusinessSalesModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->BusinessSalesModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested Business Sales post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
