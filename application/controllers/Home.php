...
class Home extends CI_Controller {
	...
	public function index()
	{
        ...
        $this->load->view('html/indexpage', $data);
		...
		public function index1()
	{
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/index1',$data);
		$this->load->view('includes/footer');
	}
			...
			$header['user_id'] = $this->session->userdata('user_id');
				...
				else
				{
				        $comment_id = $this->uri->segment(3);
			       		$data = array();
						$comment=$this->DiscussionModel->get_comments($comment_id);		
						$data['comment']=$comment[0];
						$this->load->view('includes/header',$header);
						$this->load->view('html/discussion',$data);
						$this->load->view('includes/footer');
				}
...
$this->db->query('delete from group_members');
...
/*$this->db->query("CREATE TABLE IF NOT EXISTS `groups` (
  ...
  `g_createddate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `g_image` varchar(255) NULL,
  `user_id` int(11) NOT NULL,
  `g_city` varchar(255)  NULL,
  `g_zipcode` varchar(255)  NULL
...
/*$this->db->query("CREATE TABLE IF NOT EXISTS `group_members` (
  `gm_id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `g_fkid` int(11) NOT NULL,
  `g_member_id` int(11) NOT NULL,
  `g_join_date` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `g_status` int(11) NOT NULL DEFAULT '1'
) ENGINE=InnoDB DEFAULT CHARSET=latin1;");

*/
/*
$this->db->query("CREATE TABLE IF NOT EXISTS `group_discussion` (
  `gd_id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `g_fkid` int(11) NOT NULL,
  `gd_message` text NOT NULL,
  `gd_member_id` int(11) NOT NULL DEFAULT '1',
  `gd_date` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP
...
/*return $this->db->query("CREATE TABLE IF NOT EXISTS `comments` (
...
/*
$this->db->query("INSERT INTO `comments` ( `parent_comment_id`, `comment`, `date`, `group_id`, `commentor_id`) VALUES ( '0', 'this is a test 1', CURRENT_TIMESTAMP, '0', '17')");
*/
...
echo "</pre>";
*/
//$this->UserModel->user_detail($this->session->userdata('user_id');

//$this->UserModel->fetch_count_professional_listings($this->session->userdata('user_id'), 'events' );
		//	 print_r($this->UserModel->user_detail($this->session->userdata('user_id'));
...
*/	}
    ...
    public function save_reviews()
    {
        ...
        if ($this->input->post('score_comfort')) {
            $rating_value = $this->input->post('score_comfort');
        } elseif ($this->input->post('score_location')) {
            $rating_value = $this->input->post('score_location');
        } elseif ($this->input->post('score_facilities')) {
            $rating_value = $this->input->post('score_facilities');
        } elseif ($this->input->post('score_staff')) {
            $rating_value = $this->input->post('score_staff');
        } elseif ($this->input->post('score_value')) {
            $rating_value = $this->input->post('score_value');
        } else {
            $rating_value = 0;
        }
        $dataReview = array(
        ...
        $result = $this->db->insert('reviews', $dataReview);
        if ($result) {
            $this->session->set_flashdata("success_message", "Reviews submited successfully.");
            $post_id = $this->input->post('post_id');
            $user_id = $this->session->userdata('user_id');
            $type = $this->input->post('type');
            $data['reviews'] = $this->UserModel->getReview($post_id, $user_id, $type);
            $this->load->view('html/review_show', $data, true);
            redirect($_POST['url']);
        } else {
            $this->session->set_flashdata("error_message", "Something occured while sendiing message.");
            redirect($_POST['url']);
