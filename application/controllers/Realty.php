...
class Realty extends CI_Controller {
    ...
    public function index()
	{
		...
		$this->load->view('html/realty-page',$user);
    ...
    public function insert(){
        ...
        $this->form_validation->set_rules('realty-category', 'Category', 'required');
        $this->form_validation->set_rules('realty-type', 'Type', 'required');
        ...
        $this->form_validation->set_rules('realty-bed', 'Bed', 'required|numeric|greater_than[0]|max_length[10]');
        $this->form_validation->set_rules('realty-bath', 'Bath', 'required|numeric|greater_than[0]|max_length[10]');
        $this->form_validation->set_rules('realty-lot', 'Lot', 'required|min_length[3]|max_length[255]');
        $this->form_validation->set_rules('realty-sewerage', 'Sewerage', 'required|min_length[3]|max_length[255]');
        $this->form_validation->set_rules('realty-water', 'Water', 'required|min_length[3]|max_length[255]');
        $this->form_validation->set_rules('realty-gas', 'Gas', 'required|min_length[3]|max_length[255]');
        $this->form_validation->set_rules('realty-tax', 'Tax', 'required|numeric|greater_than[0]|max_length[10]');
        $this->form_validation->set_rules('realty-mls', 'MLS', 'required|min_length[3]|max_length[255]');
        ...
        else
        {
            $UploadPreferences = array(
                ...
                'file_name'=>$this->session->userdata('user_id')."_".time()."_realty.jpg"
            );
            ...
            $this->RealtyModel->insert_realty($path);

            echo json_encode( array('success' => "Realty Listing has been Added.....!") );
        }
    }
    ...
    public function edit(){
        ...
        else
        {
            ...
            $this->RealtyModel->edit_realty($path);

            echo json_encode( array('success' => "Realty Listing has been Edited.....!") );
        }
    }
    ...
    public function filters(){
        ...
        if( $this->input->post('index') == 0 ){
            $query = $this->RealtyModel->filter($date_array);
        }
        else{
            $query = $this->RealtyModel->loadmore_filter($date_array,$this->input->post('index'));
        }
    ...
    {   
        if( $this->input->post('index') == 0 ){
            $query = $this->RealtyModel->no_filter();
        }
        else{
            $query = $this->RealtyModel->loadmore_no_filter($this->input->post('index'));
        }
    ...
    {  
        if( $this->input->post('index') == 0 ){
            $query = $this->RealtyModel->search($this->input->post('keyword'));
        }
        else{
            $query = $this->RealtyModel->loadmore_search($this->input->post('keyword'),$this->input->post('index'));
        }
    ...
    public function post_details($id){
        ...
        $query = $this->RealtyModel->post_details($id);
		...
		$this->load->view('includes/header',$header);
		$this->load->view('html/realty-details',$data);
		$this->load->view('includes/footer');
    }
    ...
    public function delete($id = '')
    {
        ...
        {
            $post = $this->RealtyModel->post_details($id);
            $post = $post->result();
            ...
            if( sizeof( $post ) > 0 )
            {
                ...
                {
                    $this->RealtyModel->delete_post($id);
                    $this->session->set_flashdata("success_message", "Requested Realty post deleted successfully!");
                    redirect("user/my_listing");
                }
                else
                {
